...
**Goal:** Make file deletion metadata updates transactional, add a `reconsile` admin repair path for chunk reference-count integrity, and convert GC execution into client-driven repeated server-side batches.
...
- Modify: `src/service.rs`
  - add service-level `reconsile` entry point
...
- Modify: `src/control.rs`
  ...
  - add request/response types for `reconsile`
- Modify: `src/server.rs`
  - add `POST /_hugrs/service/reconsile`
  - update `POST /_hugrs/service/gc`
- Modify: `src/admin_client.rs`
  - add client methods for `reconsile`
  - update GC execute request shape
- Modify: `src/hugrsctl_cli.rs`
  - add `service reconsile`
...
- Modify: `tests/control_tests.rs`
  - add CLI command parsing coverage for `service reconsile`
- Modify: `tests/e2e_tests.rs`
  - add control-plane endpoint coverage for `reconsile` and batched GC
...
Run: `cargo test test_reconsile_chunk_refs_dry_run_reports_without_mutating --test metadata_tests`

Expected: FAIL because `reconsile_chunk_refs(...)` does not exist yet.
...
### Task 2: Implement Transactional Delete and `reconsile` in MetadataStore
...
Keep the public method name exactly `reconsile_chunk_refs` to match the requested external naming.
...
git commit -m "fix: make delete transactional and add refcount reconsile"
```

### Task 3: Expose `reconsile` Through Service, Control Types, and Server
...
Add a new control-plane test that calls `POST /_hugrs/service/reconsile` with `{"dry_run": true}` and expects the new summary response shape.
...
Run: `cargo test test_control_api_reconsile_dry_run_reports_summary --test e2e_tests`
...
- route: `POST /_hugrs/service/reconsile`
- handler that checks admin auth and returns `ReconsileResponse`

- [ ] **Step 5: Run the control-plane e2e tests to verify they pass**

Run: `cargo test --test e2e_tests test_control_api_reconsile_dry_run_reports_summary test_control_api_rejects_missing_token`

Expected: PASS

- [ ] **Step 6: Commit the control-plane `reconsile` API**

```bash
git add src/service.rs src/control.rs src/server.rs tests/e2e_tests.rs
git commit -m "feat: add chunk ref reconsile admin api"
```
...
### Task 5: Add `hugrsctl service reconsile` and Looping GC UX
...
- [ ] **Step 1: Write the failing CLI parse test for `service reconsile`**
...
fn test_service_reconsile_command_parses() {
    ...
    match cli.resource {
        Resource::Service(ServiceArgs {
            command: Some(ServiceCommand::Reconsile { dry_run: true }),
        }) => {}
...
Run: `cargo test test_service_reconsile_command_parses --test control_tests`
...
- [ ] **Step 5: Implement `service reconsile` execution path**
...
git commit -m "feat: add service reconsile and looping gc client"
```
...
Expected: diff is limited to transactional delete, `reconsile`, and batched GC changes
...
git commit -m "feat: add reconsile repair and batched gc"
```

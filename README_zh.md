...
高性能 HuggingFace 模型镜像服务。基于 prefetch + 内容寻址架构，读时 SHA256 校验数据完整性，内置分块去重与压缩，保障大模型供应链安全与高速访问。
...
- **透明代理** — 完整转发上游 headers，兼容 HF Hub 协议
...
| GET | `/files/:name` | 下载文件 |
| GET | `/files/:name/info` | 文件元数据 |
| POST | `/files/pull` | 从 HF Hub 拉取 |
